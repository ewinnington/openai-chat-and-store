...
CREATE TABLE system_prompt (id INTEGER PRIMARY KEY GENERATED ALWAYS AS IDENTITY, prompt_name TEXT NOT NULL, system_prompt TEXT NOT NULL);
INSERT INTO  system_prompt (prompt_name, system_prompt) VALUES ('default', 'Hello, I am a chatbot. I am here to help you with your questions. What would you like to know?');
...
CREATE TABLE prompt_response (id INTEGER PRIMARY KEY GENERATED ALWAYS AS IDENTITY, conversation_id INTEGER REFERENCES conversation(id), order_num INTEGER NOT NULL,  prompt JSONB NOT NULL,  response JSONB NOT NULL);